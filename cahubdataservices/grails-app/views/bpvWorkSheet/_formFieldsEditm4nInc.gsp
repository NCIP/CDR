<%@ page import="nci.obbr.cahub.forms.bpv.worksheet.Module1Sheet" %>
      ...
      <table>
         <tr><td class="name">Select Priority<span id="bpvworksheet.priority" class="vocab-tooltip"></span></td><td class="value ${hasErrors(bean: module4NSheetInstance, field: 'priority', 'errors')}"><div><g:radio id ="p1" name="priority4"  value="1" checked="${module4NSheetInstance.priority ==1}"  />&nbsp;<label for="p1">Priority I</label><br /><g:radio id ="p2" name="priority4"  value="2" checked="${module4NSheetInstance.priority ==2}"  />&nbsp;<label for="p2">Priority II</label></div></td></tr>
       </table>
      <br></br>
      
      <g:if test ="${bpvWorkSheetInstance.caseRecord.primaryTissueType.code=='OVARY'}">
       <table>
         <tr><td class="name">Module IV tissue was collected from:<span id="bpvworksheet.moduletissuefrom" class="vocab-tooltip"></span></td><td class="value ${hasErrors(bean: module4NSheetInstance, field: 'whichOvary', 'errors')}"><div><g:radio id ="l" name="whichOvary4"  value="L" checked="${module4NSheetInstance.whichOvary =='L'}"  />&nbsp;<label for="l">Left Ovary</label><br /><g:radio id ="r" name="whichOvary4"  value="R" checked="${module4NSheetInstance.whichOvary =='R'}"  />&nbsp;<label for="r">Right Ovary</label></div></td></tr>
       </table>
      </g:if>
      ...
      <div class="line">
         ...
         <table class="bpvws">
          ...
          <tr class="prop">
            ...
            <td class="${errorMap.get("dateSampleFrozenStr_" +module4NSheetInstance?.sampleFr?.id)}">
               <g:if test="${session.LDS == true}">
          <g:textField  name="dateSampleFrozenStr_${module4NSheetInstance?.sampleFr?.id}" value="${module4NSheetInstance?.sampleFr?.dateSampleFrozenStr}" />
           </g:if>
      ...
      <table class="bpvws">
        ...
        <g:if test="${session.LDS == false || session.LDS == null}">
          ...
          <td class="${warningMap?.get('interval1_' + s.id) ? 'warnings' : ''}"><span id="in1_${s.id}">${intervalMap4n.get('interval1_' + s.id)}</span></td>
          </tr>
        </g:each>
      </table>
      <br></br>
      ...
      <table class="bpvws">
        ...
        <g:each in="[module4NSheetInstance.sampleQc]" status="i" var="s">
          ...
          <td class="${warningMap?.get('interval2_' + s.id) ? 'warnings' : ''}"><span id="in2_${s.id}">${intervalMap4n.get('interval2_' + s.id)}</span></td>
         ...
         <g:if test="${session.LDS == false || session.LDS == null}">
          ...
          <td><span id="in3_${s.id}">${intervalMap4n.get('interval3_' + s.id)}</span></td>
