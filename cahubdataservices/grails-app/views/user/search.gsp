...
<div>
	...
	<g:form action='userSearch' name='userSearchForm'>
		...
		<table>
			...
			<tr>
				<td><g:message code='user.fname.label' default='First Name'/>:</td>
				<td colspan='3'><g:textField name='fname' size='50' maxlength='255' autocomplete='off' value='${fname}'/></td>
			</tr>                
			<tr>
				<td><g:message code='user.lname.label' default='Last Name'/>:</td>
				<td colspan='3'><g:textField name='lname' size='50' maxlength='255' autocomplete='off' value='${lname}'/></td>
			</tr>                
			<tr>
				<td>&nbsp;</td>
...
<%
def queryParams = [username: username, email: email, fname: fname, lname: lname, enabled: enabled, accountExpired: accountExpired, accountLocked: accountLocked, passwordExpired: passwordExpired]
%>
	...
	<table>
		...
		<tr>
			<g:sortableColumn property="username" title="${message(code: 'user.username.label', default: 'Username')}" params="${queryParams}"/>
                        ...
                        <g:sortableColumn property="fname" title="${message(code: 'user.fname.label', default: 'First Name')}" params="${queryParams}"/>
                        <g:sortableColumn property="lname" title="${message(code: 'user.lname.label', default: 'Last Name')}" params="${queryParams}"/>
...
$(document).ready(function() {
	$("#email").focus().autocomplete({
		minLength: 3,
		cache: false,
		source: "${createLink(action: 'ajaxEmailSearch')}"
	});      
	$("#fname").focus().autocomplete({
		minLength: 3,
		cache: false,
		source: "${createLink(action: 'ajaxFnameSearch')}"
	});  
	$("#lname").focus().autocomplete({
		minLength: 3,
		cache: false,
		source: "${createLink(action: 'ajaxLnameSearch')}"
	});        
	$("#username").focus().autocomplete({
		minLength: 3,
		cache: false,
		source: "${createLink(action: 'ajaxUsernameSearch')}"
	});        
});
